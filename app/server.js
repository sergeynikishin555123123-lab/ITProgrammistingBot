...
// ★★★★ ДОБАВЛЯЕМ ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ФОРМАТИРОВАНИЯ ★★★★

// Функция для извлечения построек из данных фермы
...
// Функция для извлечения культур
...
// Функция для извлечения источников воды
...
// Функция для создания начальной фермы
...
// Функция для получения простых моделей по умолчанию
...
app.get('/api/health', (req, res) => {
    res.json({ 
        ...
        lessons: lessons.getAllLessons().length,
...
app.get('/api/farm/3d/:userId', (req, res) => {
    try {
        ...
        const farm3DData = {
            ...
            cells: farm?.cells || [],
            buildings: extractBuildings(farm), // Используем функцию
            crops: extractCrops(farm),         // Используем функцию
            waterSources: extractWaterSources(farm), // Используем функцию
            stats: farm?.stats || {
                clearedLand: 0,
                buildings: 0,
                crops: 0,
...
app.get('/api/farm/:userId', (req, res) => {
    try {
        ...
        if (farm) {
            ...
            const initialFarm = createInitialFarmData(req.params.userId);
...
app.get('/api/3d/models/:modelName', (req, res) => {
    ...
    if (fs.existsSync(modelPath)) {
        ...
        const defaultModel = getDefaultModel(modelName);
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    const clientExists = fs.existsSync(path.join(__dirname, '../client'));
    const engineExists = fs.existsSync(path.join(__dirname, '../app/3d-farm-engine.js'));
    const assetsExists = fs.existsSync(path.join(__dirname, '../assets'));
    
    console.log(`🎮 3D Ферма: ${engineExists ? 'Доступна' : 'Не доступна'}`);
    console.log(`📁 Клиентские файлы: ${clientExists ? 'Найдены' : 'Не найдены'}`);
    console.log(`📦 Ассеты: ${assetsExists ? 'Найдены' : 'Не найдены'}`);
});
