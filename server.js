    ...
    extractSubscriptionInfo(lead) {
        ...
        try {
            ...
            if (subscriptionInfo.hasSubscription) {
                ...
                console.log(`   • Активация в будущем: ${isFutureActivation ? 'Да' : 'Нет'}`);
                ...
                // Логика определения статуса - ПРИОРИТЕТ ПО ДАННЫМ, А НЕ ПО СТАТУСУ СДЕЛКИ
                if (isExpiredByDate) {
                    ...
                    // Если сделка закрыта, но есть остаток - это странно, но показываем активным
                    if (isClosedDeal) {
                        console.log(`⚠️  Внимание: сделка закрыта, но есть остаток занятий!`);
                    }
                }
...
async findLatestActiveSubscription(contactId) {
    ...
    try {
        const result = await this.findActiveSubscriptions(contactId);
        
        if (result.active && result.active.length > 0) {
            const activeSub = result.active[0];
            console.log(`✅ Найден активный абонемент в сделке ID: ${activeSub.lead_id}`);
            
            // Получаем полную сделку
            const lead = await this.getLeadById(activeSub.lead_id);
            if (lead) {
                const subscriptionInfo = this.extractSubscriptionInfo(lead);
                console.log(`📊 Данные абонемента: ${subscriptionInfo.totalClasses} занятий, ${subscriptionInfo.remainingClasses} осталось`);
        ...
        } else if (result.all && result.all.length > 0) {
            console.log(`⚠️  Активных нет, берем последний абонемент`);
            const lastSub = result.all[0];
            const lead = await this.getLeadById(lastSub.lead_id);
...
async getContactLeadsOptimized(contactId, limit = 50) {
    ...
    try {
        ...
        for (const contact of contacts) {
            ...
            for (const child of children) {
                ...
                let bestLead = null;
                let bestSubscriptionInfo = this.extractSubscriptionInfo(null); // Пустой по умолчанию
                ...
                // 6. Создаем профиль с ВСЕМИ данными об абонементе
                const studentProfile = this.createStudentProfile(
                    fullContact,
                    phoneNumber,
                    child,
                    bestSubscriptionInfo, // Убедитесь, что это не пустой объект
                    bestLead
                );
                
                // ВАЖНО: Проверяем, что данные об абонементе добавлены
                if (bestSubscriptionInfo && bestSubscriptionInfo.hasSubscription) {
                    console.log(`✅ Данные абонемента добавлены в профиль:`);
                    console.log(`   Тип: ${studentProfile.subscription_type}`);
                    console.log(`   Статус: ${studentProfile.subscription_status}`);
                    console.log(`   Занятий: ${studentProfile.used_classes}/${studentProfile.total_classes}`);
                }
   ...
   createStudentProfile(contact, phoneNumber, studentInfo, subscriptionInfo, lead) {
    ...
    // Проверяем subscriptionInfo
    let finalSubscriptionInfo = subscriptionInfo;
    if (!subscriptionInfo || !subscriptionInfo.hasSubscription) {
        console.log(`⚠️  subscriptionInfo пустой или без абонемента, создаем пустой`);
        finalSubscriptionInfo = this.extractSubscriptionInfo(null); // Пустой объект
    }
    ...
    const profile = {
        ...
        // ВАЖНО: Данные абонемента - берем из finalSubscriptionInfo
        subscription_type: finalSubscriptionInfo.subscriptionType || 'Без абонемента',
        subscription_active: finalSubscriptionInfo.subscriptionActive ? 1 : 0,
        subscription_status: finalSubscriptionInfo.subscriptionStatus || 'Нет абонемента',
        subscription_badge: finalSubscriptionInfo.subscriptionBadge || 'inactive',
        total_classes: finalSubscriptionInfo.totalClasses || 0,
        remaining_classes: finalSubscriptionInfo.remainingClasses || 0,
        used_classes: finalSubscriptionInfo.usedClasses || 0,
        expiration_date: finalSubscriptionInfo.expirationDate || null,
        activation_date: finalSubscriptionInfo.activationDate || null,
        last_visit_date: finalSubscriptionInfo.lastVisitDate || null,
...
async getContactLeadsAlternative(contactId) {
    ...
    async findSubscriptionForStudent(contactId, studentName) {
        console.log(`\n🎯 ПОИСК АБОНЕМЕНТА ДЛЯ: ${studentName} (контакт: ${contactId})`);
        ...
        try {
            ...
            // 2. Если сделок нет или мало, возможно проблема с API
            if (allLeads.length < 10) {
                console.log(`⚠️  Мало сделок (${allLeads.length}). Возможно проблема с пагинацией или фильтрацией.`);
                
                // Пробуем альтернативный запрос без фильтра по контакту
                console.log(`🔍 Альтернативный поиск по имени ученика...`);
                const altLeads = await this.searchLeadsByStudentName(studentName);
                if (altLeads.length > 0) {
                    console.log(`📊 Найдено альтернативным способом: ${altLeads.length} сделок`);
                    allLeads.push(...altLeads);
                }
            }
            ...
            for (const lead of allLeads) {
                ...
                if (hasSubscription) {
                    // Проверяем, относится ли сделка к этому ученику
                    const isForThisStudent = this.isLeadForStudent(lead, studentName);
                    
                    if (isForThisStudent) {
                        console.log(`✅ Найдена подходящая сделка: ${lead.id} "${lead.name}"`);
                        subscriptionLeads.push(lead);
                    } else {
                        console.log(`📋 Сделка с абонементом, но не для ${studentName}: ${lead.id} "${lead.name}"`);
                    }
                }
            }
            
            console.log(`📊 Подходящих сделок с абонементами: ${subscriptionLeads.length}`);
            
            // 4. Выбираем самую актуальную
            if (subscriptionLeads.length > 0) {
                // Сортируем по дате активации, затем по дате создания
                subscriptionLeads.sort((a, b) => {
                    const dateA = this.getLeadActivationDate(a) || a.created_at || 0;
                    const dateB = this.getLeadActivationDate(b) || b.created_at || 0;
                    return dateB - dateA; // Новые сначала
                });
                
                const bestLead = subscriptionLeads[0];
                console.log(`🎯 Выбрана самая актуальная сделка: ${bestLead.id} "${bestLead.name}"`);
            ...
            console.log(`❌ Не найдено подходящих сделок с абонементами`);
            return null;
        ...
        try {
            ...
            for (const lead of allLeads) {
                ...
                if (subscriptionInfo.hasSubscription) {
                    ...
                    console.log(`   Всего: ${subscriptionInfo.totalClasses}, Использовано: ${subscriptionInfo.usedClasses}, Осталось: ${subscriptionInfo.remainingClasses}`);
                    console.log(`   Активен в системе: ${subscriptionInfo.subscriptionActive ? '✅ Да' : '❌ Нет'}`);
                    
                    // ВАЖНО: Проверяем по реальным данным, а не только по статусу
                    const hasActiveData = 
                        (subscriptionInfo.remainingClasses > 0 || 
                         subscriptionInfo.totalClasses > subscriptionInfo.usedClasses) &&
                    ...
                    const subscriptionData = {
                        ...
                        subscription: subscriptionInfo,
                        is_active_by_data: hasActiveData,
                    ...
                    // Считаем активными те, у которых есть остаток занятий И не истек срок
                    if (hasActiveData) {
                        activeSubscriptions.push(subscriptionData);
                        console.log(`   🎯 АКТИВНЫЙ ПО ДАННЫМ!`);
                    } else if (subscriptionInfo.subscriptionActive) {
                        // Если система считает активным, но данные устарели
                        console.log(`   ⚠️  Система считает активным, но данные: остаток=${subscriptionInfo.remainingClasses}, истек=${!subscriptionInfo.expirationDate || new Date(subscriptionInfo.expirationDate) < new Date()}`);
                    }
                }
            }
            ...
            console.log(`   Активных по данным: ${activeSubscriptions.length}`);
            
            // СОРТИРОВКА по актуальности:
            // 1. Сначала активные по данным
            // 2. Потом по дате активации (новые сначала)
            // 3. Потом по дате создания (новые сначала)
            
            activeSubscriptions.sort((a, b) => {
                // Сначала по активности
                if (a.is_active_by_data !== b.is_active_by_data) {
                    return b.is_active_by_data - a.is_active_by_data;
                }
                
                // Потом по дате активации (новые сначала)
                const dateA = a.activation_date ? new Date(a.activation_date) : new Date(0);
                const dateB = b.activation_date ? new Date(b.activation_date) : new Date(0);
                if (dateB.getTime() !== dateA.getTime()) {
                    return dateB.getTime() - dateA.getTime();
                }
                
                // Потом по дате обновления
                return new Date(b.updated_at) - new Date(a.updated_at);
            });
            
            // Также сортируем все абонементы для отладки
            ...
            return { active: [], all: [], total: 0, active_count: 0 };
        }
    }
    ...
    try {
        ...
        for (const contact of contacts) {
            ...
            for (const child of children) {
                ...
                // Ищем абонемент
                const bestLead = await this.findSubscriptionForStudent(contact.id, child.studentName);
...
// Маршрут для отладки создания профиля
app.get('/api/debug/profile-creation/:phone', async (req, res) => {
    try {
        const phone = req.params.phone;
        console.log(`\n🔧 ОТЛАДКА СОЗДАНИЯ ПРОФИЛЯ ДЛЯ: ${phone}`);
        ...
        for (const contact of contacts.slice(0, 2)) { // Первые 2 контакта
            console.log(`\n👤 Контакт: ${contact.name} (ID: ${contact.id})`);
            ...
            // Ищем детей
            const children = amoCrmService.extractStudentsFromContact(fullContact);
            console.log(`   👶 Детей: ${children.length}`);
            ...
            for (const child of children) {
                ...
                const studentProfile = amoCrmService.createStudentProfile(
                    fullContact,
                    formattedPhone,
                    child,
                    subscriptionData?.subscription || amoCrmService.extractSubscriptionInfo(null),
                    subscriptionData?.lead || null
                );
                ...
                debugInfo.push({
                    ...
                    has_subscription: subscriptionData ? true : false,
                    subscription_data: subscriptionData?.subscription || null,
                    created_profile: {
                        student_name: studentProfile.student_name,
                        branch: studentProfile.branch,
                        subscription_type: studentProfile.subscription_type,
                        subscription_status: studentProfile.subscription_status,
                        total_classes: studentProfile.total_classes,
                        used_classes: studentProfile.used_classes,
                        remaining_classes: studentProfile.remaining_classes,
                        subscription_active: studentProfile.subscription_active
                    }
                });
                
                console.log(`   📊 Создан профиль:`);
                console.log(`      Абонемент: ${studentProfile.subscription_type}`);
                console.log(`      Статус: ${studentProfile.subscription_status}`);
                console.log(`      Активен: ${studentProfile.subscription_active === 1 ? 'Да' : 'Нет'}`);
                console.log(`      Занятий: ${studentProfile.used_classes}/${studentProfile.total_classes}`);
            }
        }
