...
// ==================== ГЛУБОКИЙ ПОИСК ВСЕХ СДЕЛОК CRM ====================
app.get('/api/deep-search/:studentName', async (req, res) => {
    try {
        const studentName = decodeURIComponent(req.params.studentName);
        
        console.log(`\n🔍 ГЛУБОКИЙ ПОИСК ВСЕХ СДЕЛОК ДЛЯ: "${studentName}"`);
        console.log('='.repeat(80));
        
        const startTime = Date.now();
        const foundLeads = [];
        
        // 1. Ищем сделки через фильтр по названию (более надежно, чем query)
        console.log('\n🔍 Метод 1: Поиск через фильтр с разными вариантами имени');
        
        // Разные варианты поискового запроса
        const searchVariants = [
            studentName,
            studentName.split(' ')[0], // Только имя
            studentName.split(' ')[1], // Только фамилия
            studentName.replace(' ', '%20'), // С пробелом как %20
            encodeURIComponent(studentName) // Полностью закодированное
        ];
        ...
        for (const searchTerm of searchVariants) {
            ...
            try {
                ...
                // Метод A: Через filter[query]
                try {
                    const response = await amoCrmService.makeRequest('GET', 
                        `/api/v4/leads?filter[query]=${searchTerm}&limit=100&with=custom_fields_values`
                    );
                    
                    if (response && response._embedded && response._embedded.leads) {
                        console.log(`      📊 Найдено через filter[query]: ${response._embedded.leads.length}`);
                    ...
                    console.log(`      ❌ filter[query] не сработал: ${queryError.message}`);
                }
                
                // Метод B: Просто получаем все сделки и фильтруем локально
                try {
                    const response = await amoCrmService.makeRequest('GET', 
                        `/api/v4/leads?limit=250&with=custom_fields_values`
                    );
                    
                    if (response && response._embedded && response._embedded.leads) {
                        console.log(`      📊 Получено всех сделок: ${response._embedded.leads.length}`);
                        ...
                        for (const lead of response._embedded.leads) {
                            ...
                            const matches = [
                                ...
                                leadName.includes(studentFirstName),
                                studentLastName && leadName.includes(studentLastName),
                                leadName.includes('полина') && leadName.includes('кунахович'),
                                leadName.includes('кунахович')
                            ];
                            
                            if (matches.some(match => match)) {
                                console.log(`      ✅ Совпадение: "${lead.name}"`);
                                ...
                                if (subscriptionInfo.hasSubscription) {
                                    foundLeads.push({
                                        lead: lead,
                                        subscriptionInfo: subscriptionInfo,
                                        search_method: 'all_leads_filtered',
                                        match_type: 'name_match'
                                    });
                                }
                            }
                    ...
                    console.log(`      ❌ Ошибка получения всех сделок: ${allError.message}`);
                }
                
            } catch (variantError) {
                console.log(`      ❌ Ошибка поиска по варианту: ${variantError.message}`);
            }
        }
        
        // 2. Прямой запрос конкретной сделки по известному ID
        console.log('\n🔍 Метод 2: Прямой запрос известных сделок');
        
        const knownLeadIds = [
            28674745, // Полина Кунахович - 8 занятий
            28674541, // Петрова Даша - 4 занятия  
            28677839  // Семен Окороков - 8 занятий
        ];
        ...
        for (const leadId of knownLeadIds) {
            try {
                ...
                if (lead) {
                    ...
                    // Проверяем, подходит ли эта сделка для нашего ученика
                    if (leadName.includes(normalizedStudentName) || 
                        normalizedStudentName.includes(leadName.split(' ')[0])) {
                        
                        console.log(`✅ Прямой запрос сделки ${leadId}: "${lead.name}"`);
                        ...
                        foundLeads.push({
                            lead: lead,
                            subscriptionInfo: subscriptionInfo,
                            search_method: 'direct_by_id',
                            lead_id: leadId
                        });
                    }
                }
            } catch (directError) {
                console.log(`❌ Ошибка прямого запроса ${leadId}: ${directError.message}`);
            }
        }
        
        // 3. Ищем в воронке абонементов
        console.log(`\n🔍 Метод 3: Поиск в воронке абонементов (ID: ${amoCrmService.SUBSCRIPTION_PIPELINE_ID})`);
        ...
        try {
            ...
            if (response && response._embedded && response._embedded.leads) {
                console.log(`📊 Найдено в воронке абонементов: ${response._embedded.leads.length}`);
                ...
                for (const lead of response._embedded.leads) {
                    ...
                    // Проверяем, есть ли имя ученика в названии
                    if (leadName.includes(normalizedStudentName) || 
                        normalizedStudentName.includes(leadName.split(' ')[0])) {
                        
                        console.log(`✅ Найдена в воронке: "${lead.name}"`);
                        ...
                        if (subscriptionInfo.hasSubscription) {
                            foundLeads.push({
                                lead: lead,
                                subscriptionInfo: subscriptionInfo,
                                search_method: 'subscription_pipeline',
                                pipeline_id: amoCrmService.SUBSCRIPTION_PIPELINE_ID
                            });
                        }
                    }
                }
            ...
            console.log(`❌ Ошибка поиска в воронке: ${pipelineError.message}`);
        }
        
        console.log(`\n📊 Всего найдено подходящих сделок: ${foundLeads.length}`);
        
        // 4. Обрабатываем найденные сделки
        const results = [];
        
        for (const foundLead of foundLeads) {
            try {
                // Получаем контакты сделки
                const contactsResponse = await amoCrmService.makeRequest('GET', 
                    `/api/v4/leads/${foundLead.lead.id}/contacts`
                    ...
                    if (contact) {
                        ...
                        results.push({
                            ...
                            lead: {
                                id: foundLead.lead.id,
                                name: foundLead.lead.name,
                                pipeline_id: foundLead.lead.pipeline_id,
                                status_id: foundLead.lead.status_id
                            },
                            subscription_info: foundLead.subscriptionInfo,
                            search_method: foundLead.search_method
                        });
                        
                        console.log(`✅ Создан профиль для: ${extractedName} (${contact.name})`);
                    }
                }
            } catch (leadError) {
                console.error(`❌ Ошибка обработки сделки:`, leadError.message);
            }
        }
        ...
        res.json({
            ...
            data: {
                ...
                total_leads_found: foundLeads.length,
                total_profiles_created: results.length,
                
                // Ключевая информация
                key_info: {
                    // Что искали
                    search_for: studentName,
                    
                    // Что нашли
                    found_in_crm: foundLeads.length > 0,
                    
                    // Если нашли
                    leads_info: foundLeads.map(lead => ({
                        id: lead.lead.id,
                        name: lead.lead.name,
                        pipeline_id: lead.lead.pipeline_id,
                        status_id: lead.lead.status_id,
                        has_subscription: lead.subscriptionInfo.hasSubscription,
                        total_classes: lead.subscriptionInfo.totalClasses,
                        remaining_classes: lead.subscriptionInfo.remainingClasses,
                        search_method: lead.search_method
                    })),
                ...
                diagnostic: {
                    search_methods_used: [
                        'filter[query] с разными вариантами',
                        'get_all_leads_and_filter_locally',
                        'direct_by_known_ids',
                        'search_in_subscription_pipeline'
                    ],
                    subscription_pipeline_id: amoCrmService.SUBSCRIPTION_PIPELINE_ID,
                    known_lead_ids_tested: knownLeadIds
                },
                
                // Результаты для приложения
                for_app: results.length > 0 ? {
                    ready_to_use: true,
                    profiles_count: results.length,
                    // Первый профиль для быстрого использования
                    first_profile: results[0] ? {
                        student_name: results[0].profile.student_name,
                        phone: results[0].contact.phone,
                        subscription_type: results[0].subscription_info.subscriptionType,
                        total_classes: results[0].subscription_info.totalClasses,
                        remaining_classes: results[0].subscription_info.remainingClasses
                    } : null,
                    // Все профили
        ...
        console.error('❌ Ошибка глубокого поиска:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message,
            stack: error.stack 
...
app.get('/api/test-lead-28674745', async (req, res) => {
    try {
        console.log(`\n🧪 ТЕСТ СДЕЛКИ 28674745 (Полина Кунахович)`);
        console.log('='.repeat(80));
        
        // 1. Прямой запрос сделки
        const lead = await amoCrmService.makeRequest('GET', 
            `/api/v4/leads/28674745?with=custom_fields_values`
        );
        
        if (!lead) {
            return res.json({ success: false, error: 'Сделка 28674745 не найдена' });
        }
        ...
        // 3. Извлекаем информацию об абонементе
        const subscriptionInfo = amoCrmService.extractSubscriptionInfo(lead);
        ...
        console.log(`   🏷️  Тип: ${subscriptionInfo.subscriptionType}`);
        console.log(`   ✅ Активен: ${subscriptionInfo.subscriptionActive ? 'Да' : 'Нет'}`);
        
        // 4. Получаем контакт
        const contactsResponse = await amoCrmService.makeRequest('GET', 
            `/api/v4/leads/28674745/contacts`
        );
        ...
        // 5. Создаем тестовый профиль
        const studentName = amoCrmService.extractStudentNameFromLead(lead.name) || 'Полина Кунахович';
        ...
        const studentInfo = {
            ...
            teacherName: '',
            ageGroup: subscriptionInfo.ageGroup || '',
            parentName: contactInfo?.name || 'Родитель',
            email: ''
        };
        
        const profile = amoCrmService.createStudentProfile(
            contactInfo || { id: 0, name: 'Тестовый контакт', custom_fields_values: [] },
            contactInfo?.phone || '+79161916984',
            studentInfo,
            subscriptionInfo,
            lead
        );
        ...
        res.json({
            ...
            data: {
                ...
                api_endpoints: {
                    for_app: `/api/by-lead-id/28674745/${contactInfo?.phone || '79660587744'}`,
                    diagnostic: `/api/debug/lead-details/28674745`,
                    deep_search: `/api/deep-search/Полина Кунахович`
                }
            }
        });
        
    } catch (error) {
        console.error('❌ Ошибка теста сделки:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message,
            stack: error.stack 
...
app.get('/api/debug/student-full-diagnostic/:phone/:studentName', async (req, res) => {
    try {
        const phone = req.params.phone;
        const studentName = decodeURIComponent(req.params.studentName);
        
        console.log(`\n🔍 ПОЛНАЯ ДИАГНОСТИКА УЧЕНИКА: "${studentName}" (${phone})`);
        console.log('='.repeat(100));
        ...
        const log = (step, message, data = null) => {
            ...
            diagnosticLog.push({ step, message, data, timestamp: new Date().toISOString() });
        };
        
        // 1. ФОРМАТИРОВАНИЕ ТЕЛЕФОНА
        const formattedPhone = formatPhoneNumber(phone);
        log('Шаг 1', 'Форматирование телефона', { original: phone, formatted: formattedPhone });
        
        // 2. ПОИСК КОНТАКТОВ
        log('Шаг 2', 'Поиск контактов по телефону');
        
        const contactsResponse = await amoCrmService.searchContactsByPhone(formattedPhone);
        const contacts = contactsResponse._embedded?.contacts || [];
        
        log('Шаг 2', 'Результаты поиска контактов', {
            found: contacts.length,
            contacts: contacts.map(c => ({ id: c.id, name: c.name }))
        });
        ...
        const contact = contacts[0];
        log('Шаг 3', 'Основной контакт', { id: contact.id, name: contact.name });
        
        // 3. ПОЛНАЯ ИНФОРМАЦИЯ О КОНТАКТЕ
        log('Шаг 4', 'Получение полной информации о контакте');
        ...
        // 4. УЧЕНИКИ В КОНТАКТЕ
        log('Шаг 5', 'Извлечение учеников из контакта');
        
        const students = amoCrmService.extractStudentsFromContact(fullContact);
        log('Шаг 5', 'Ученики в контакте', {
        ...
        log('Шаг 5', 'Поиск целевого ученика', {
            looking_for: studentName,
            normalized: normalizedStudentName,
            found: !!targetStudent,
            student_data: targetStudent || null
        });
        ...
        // 5. ПОИСК СДЕЛОК КОНТАКТА
        log('Шаг 6', 'Поиск всех сделок контакта');
        
        const allLeads = await amoCrmService.getContactLeadsFixed(contact.id);
        log('Шаг 6', 'Все сделки контакта', {
            ...
            leads: allLeads.slice(0, 10).map(l => ({
                ...
                pipeline_id: l.pipeline_id,
                status_id: l.status_id,
                created: new Date(l.created_at * 1000).toLocaleDateString()
            }))
        });
        
        // 6. ПОИСК СДЕЛКИ ДЛЯ УЧЕНИКА
        log('Шаг 7', 'Поиск сделки для ученика');
        
        let foundLead = null;
        let searchMethod = 'NOT_FOUND';
        
        // Метод 1: Поиск по имени в сделках
        for (const lead of allLeads) {
            const leadName = amoCrmService.normalizeName(lead.name);
            ...
            if (leadName.includes(normalizedStudentName) || 
                ...
                foundLead = lead;
                searchMethod = 'NAME_MATCH_IN_LEAD';
                log('Шаг 7.1', 'Найдена сделка по имени', {
                    lead_id: lead.id,
                    lead_name: lead.name,
                    match_type: 'Имя ученика в названии сделки'
                });
                break;
            }
        }
        
        // Метод 2: Поиск в воронке абонементов
        if (!foundLead) {
            log('Шаг 7.2', 'Поиск в воронке абонементов', {
                pipeline_id: amoCrmService.SUBSCRIPTION_PIPELINE_ID
            });
            ...
            for (const lead of allLeads) {
                if (lead.pipeline_id === amoCrmService.SUBSCRIPTION_PIPELINE_ID) {
                    foundLead = lead;
                    searchMethod = 'SUBSCRIPTION_PIPELINE';
                    log('Шаг 7.2', 'Найдена сделка в воронке абонементов', {
                        lead_id: lead.id,
                        lead_name: lead.name
                    });
                    break;
        ...
        // Метод 3: Проверка известных ID
        if (!foundLead) {
            log('Шаг 7.3', 'Проверка известных ID сделок для этого ученика');
            
            // Проверим, есть ли сделка с таким именем в системе
            try {
                const searchTerm = encodeURIComponent(studentName.split(' ')[0]);
                ...
                if (searchResponse && searchResponse._embedded && searchResponse._embedded.leads) {
                    for (const lead of searchResponse._embedded.leads) {
                        ...
                        if (leadName.includes(normalizedStudentName) || 
                            ...
                            foundLead = lead;
                            searchMethod = 'GLOBAL_SEARCH';
                            log('Шаг 7.3', 'Найдена сделка глобальным поиском', {
                                lead_id: lead.id,
                                lead_name: lead.name
                            });
                            break;
                ...
                log('Шаг 7.3', 'Глобальный поиск не сработал', { error: searchError.message });
            }
        }
        
        // 7. АНАЛИЗ НАЙДЕННОЙ СДЕЛКИ
        let subscriptionInfo = null;
        let hasSubscription = false;
        
        if (foundLead) {
            log('Шаг 8', 'Анализ найденной сделки', {
            ...
            log('Шаг 8', 'Результат анализа абонемента', {
                has_subscription: subscriptionInfo.hasSubscription,
                subscription_active: subscriptionInfo.subscriptionActive,
                total_classes: subscriptionInfo.totalClasses,
            ...
            // Показываем ВСЕ поля сделки
            const customFields = foundLead.custom_fields_values || [];
            log('Шаг 8.1', 'Все поля сделки', {
                total_fields: customFields.length,
                fields: customFields.map(f => ({
                    id: f.field_id || f.id,
                    name: amoCrmService.getFieldNameById(f.field_id || f.id),
                    value: amoCrmService.getFieldValue(f),
                    is_subscription: amoCrmService.getFieldNameById(f.field_id || f.id)?.toLowerCase().includes('абонемент') ||
                                   amoCrmService.getFieldNameById(f.field_id || f.id)?.toLowerCase().includes('занят')
                }))
            });
        } else {
            log('Шаг 8', 'Сделка не найдена', { error: 'Не удалось найти сделку для ученика' });
        }
        
        // 8. СОЗДАНИЕ ПРОФИЛЯ
        log('Шаг 9', 'Создание профиля ученика');
        ...
        if (foundLead && subscriptionInfo) {
            ...
            log('Шаг 9', 'Профиль создан из сделки', {
                student_name: profile.student_name,
                total_classes: profile.total_classes,
                remaining_classes: profile.remaining_classes,
                subscription_type: profile.subscription_type,
            ...
            log('Шаг 9', 'Профиль создан без сделки', {
                student_name: profile.student_name,
                total_classes: profile.total_classes,
                warning: 'В профиле 0 занятий, так как сделка не найдена'
            });
        }
        
        // 9. ПРОВЕРКА В БАЗЕ ДАННЫХ
        log('Шаг 10', 'Проверка в базе данных');
        
        const dbProfile = await db.get(
            `SELECT * FROM student_profiles 
             WHERE student_name = ? AND phone_number LIKE ?`,
            [studentName, `%${formattedPhone.slice(-10)}%`]
        );
        
        log('Шаг 10', 'Результат проверки в БД', {
            found_in_db: !!dbProfile,
            profile_in_db: dbProfile ? {
                id: dbProfile.id,
        ...
        // 10. АНАЛИЗ ПРОБЛЕМЫ
        log('Шаг 11', 'АНАЛИЗ ПРОБЛЕМЫ');
        
        const problemAnalysis = [];
        
        if (!foundLead) {
            problemAnalysis.push({
                problem: 'Сделка не найдена для ученика',
                reason: 'В названиях сделок контакта нет имени ученика',
                solution: 'Проверьте название сделок в amoCRM - должно содержать имя ученика'
            });
        } else if (!hasSubscription) {
            problemAnalysis.push({
                problem: 'В сделке нет данных об абонементе',
                reason: 'Поля "Всего занятий", "Остаток занятий" не заполнены или не найдены',
                solution: 'Заполните поля абонемента в сделке в amoCRM'
            });
        } else if (profile.total_classes === 0) {
            problemAnalysis.push({
                problem: 'В профиле 0 занятий',
                reason: 'Метод extractSubscriptionInfo не извлек данные об абонементе',
                solution: 'Проверьте правильность ID полей в FIELD_IDS'
            });
        }
        
        log('Шаг 11', 'Анализ проблем', { problems: problemAnalysis.length, details: problemAnalysis });
        ...
        res.json({
            ...
            results: {
                ...
                student_found: !!targetStudent,
                student: targetStudent ? {
                    name: targetStudent.studentName,
                    branch: targetStudent.branch,
                    teacher: targetStudent.teacherName,
                    age_group: targetStudent.ageGroup
                } : null,
                
                // Сделка
                lead_found: !!foundLead,
                lead: foundLead ? {
                    id: foundLead.id,
                    name: foundLead.name,
                    pipeline_id: foundLead.pipeline_id,
                    status_id: foundLead.status_id,
                    search_method: searchMethod
                } : null,
                
                // Абонемент
                subscription_found: hasSubscription,
                ...
                // В базе данных
                in_database: !!dbProfile
            },
            
            // Ключевые проблемы
            key_problems: problemAnalysis,
            ...
            recommendations: (() => {
                ...
                if (!foundLead) {
                    recs.push('🚨 ПРОБЛЕМА: Сделка не найдена');
                    recs.push('   🔧 Проверьте названия сделок в amoCRM - они должны содержать имя ученика');
                    recs.push('   🔧 Проверьте воронку абонементов (ID: ' + amoCrmService.SUBSCRIPTION_PIPELINE_ID + ')');
                    recs.push('   🔧 Вручную найдите сделку для "' + studentName + '" в amoCRM');
                }
                
                if (foundLead && !hasSubscription) {
                    recs.push('🚨 ПРОБЛЕМА: В сделке нет данных об абонементе');
                    recs.push('   🔧 Проверьте поля в сделке:');
                    recs.push('        - "Абонемент занятий:" (ID: ' + amoCrmService.FIELD_IDS.LEAD.TOTAL_CLASSES + ')');
                ...
                if (profile && profile.total_classes === 0) {
                    recs.push('🚨 ПРОБЛЕМА: В профиле 0 занятий');
                    recs.push('   🔧 Проверьте метод extractSubscriptionInfo');
                    recs.push('   🔧 Проверьте правильность ID полей в FIELD_IDS');
                }
            ...
            // Быстрые действия
            quick_actions: [
                `GET /api/simple-test/[ID_СДЕЛКИ] - Создать профиль по ID сделки`,
                `GET /api/debug/lead-fields/[ID_СДЕЛКИ] - Посмотреть все поля сделки`,
...
// ==================== БЫСТРЫЙ ПОИСК СДЕЛКИ ПО ИМЕНИ УЧЕНИКА ====================
app.get('/api/find-lead-by-student/:studentName', async (req, res) => {
    try {
        const studentName = decodeURIComponent(req.params.studentName);
        
        console.log(`\n🔍 БЫСТРЫЙ ПОИСК СДЕЛКИ ДЛЯ: "${studentName}"`);
        console.log('='.repeat(80));
        ...
        console.log(`🔍 Поиск по: "${firstName}" "${lastName}"`);
        
        let foundLeads = [];
        
        // Пробуем разные варианты поиска
        const searchTerms = [firstName, lastName, studentName].filter(term => term && term.length > 2);
        
        for (const term of searchTerms) {
            try {
                const response = await amoCrmService.makeRequest('GET', 
                    `/api/v4/leads?filter[query]=${encodeURIComponent(term)}&limit=50&with=custom_fields_values`
                );
                ...
                console.log(`❌ Поиск по "${term}" не сработал:`, searchError.message);
            }
        }
        ...
        console.log(`📊 Найдено уникальных сделок: ${uniqueLeads.length}`);
        ...
        if (uniqueLeads.length === 0) {
            return res.json({
                ...
                suggestions: [
                    '1. Проверьте правильность написания имени и фамилии',
                    '2. Проверьте, есть ли сделка с именем ученика в amoCRM',
                    '3. Используйте прямой поиск в сделках amoCRM'
                ]
            });
        }
        ...
        res.json({
            ...
            data: {
                ...
                leads: uniqueLeads,
                best_match: uniqueLeads[0], // Первая сделка - самая подходящая
                actions: [
                    `GET /api/simple-test/${uniqueLeads[0].id} - Создать профиль`,
                    `GET /api/debug/lead-fields/${uniqueLeads[0].id} - Посмотреть поля сделки`,
                    `GET /api/debug/student-full-diagnostic/[ТЕЛЕФОН]/${encodeURIComponent(studentName)} - Полная диагностика`
                ]
            }
        });
...
app.get('/api/simple-test/:leadId', async (req, res) => {
    try {
        const leadId = req.params.leadId;
        ...
        // 2. Просто извлекаем данные об абонементе
        const subscriptionInfo = amoCrmService.extractSubscriptionInfo(lead);
        
        // 3. Создаем УПРОЩЕННЫЙ профиль БЕЗ контакта
        const simplifiedProfile = {
            student_name: amoCrmService.extractStudentNameFromLead(lead.name) || 'Ученик',
            phone_number: '+79161916984', // Тестовый телефон
            branch: amoCrmService.getLeadBranch(lead) || 'Филиал не указан',
            subscription_type: subscriptionInfo.subscriptionType || 'Без абонемента',
            subscription_active: subscriptionInfo.subscriptionActive ? 1 : 0,
            subscription_status: subscriptionInfo.subscriptionStatus || 'Не активен',
        ...
        // 4. Сохраняем в БД
        const savedCount = await saveProfilesToDatabase([simplifiedProfile]);
        
        res.json({
            success: true,
            message: 'Тестовый профиль создан',
            data: {
                lead: {
                    id: lead.id,
                    name: lead.name,
...
// ==================== ТЕСТОВЫЙ МАРШРУТ ДЛЯ ВСЕХ ПРОБЛЕМНЫХ УЧЕНИКОВ ====================
app.get('/api/test-problem-students', async (req, res) => {
    try {
        console.log(`\n🧪 ТЕСТ ВСЕХ ПРОБЛЕМНЫХ УЧЕНИКОВ`);
        console.log('='.repeat(80));
        
        const testStudents = [
            { name: 'Иван Юрлов', phone: '79660587744', expected_lead: 28674703 },
            { name: 'Саша Мясоедова', phone: '79660587744', expected_lead: 28674177 },
            { name: 'Серик Артемий', phone: '79660587744', expected_lead: 28668043 },
            { name: 'Полина Кунахович', phone: '79660587744', expected_lead: 28674745 },
            { name: 'Захар Веребрюсов', phone: '79660587744', expected_lead: 28677839 },
            { name: 'Петрова Даша', phone: '79660587744', expected_lead: 28674541 },
            { name: 'Семен Окороков', phone: '79660587744', expected_lead: 28677839 }
        ];
        ...
        for (const student of testStudents) {
            try {
                console.log(`\n🔍 Тест: "${student.name}" (${student.phone})`);
                
                // Имитируем запрос к исправленному маршруту
                const mockReq = {
                    body: {
                        phone: student.phone,
                ...
                // Пробуем найти сделку
                const searchResponse = await amoCrmService.makeRequest('GET', 
                    `/api/v4/leads?filter[query]=${encodeURIComponent(student.name.split(' ')[0])}&limit=10&with=custom_fields_values`
                );
                ...
                if (searchResponse && searchResponse._embedded && searchResponse._embedded.leads) {
                    for (const lead of searchResponse._embedded.leads) {
                        const leadName = amoCrmService.normalizeName(lead.name);
                        const studentName = amoCrmService.normalizeName(student.name);
                        
                        if (leadName.includes(studentName) || studentName.includes(leadName.split(' ')[0])) {
                            const subscriptionInfo = amoCrmService.extractSubscriptionInfo(lead);
                            
                            if (subscriptionInfo.hasSubscription) {
                                leadFound = true;
                                totalClasses = subscriptionInfo.totalClasses;
                                break;
                            }
                        }
                    }
                ...
                // Проверяем известный ID
                if (!leadFound) {
                    try {
                        const lead = await amoCrmService.makeRequest('GET', 
                            `/api/v4/leads/${student.expected_lead}?with=custom_fields_values`
                        );
                ...
                results.push({
                    ...
                    phone: student.phone,
                    expected_lead: student.expected_lead,
                    lead_found: leadFound,
                    total_classes: totalClasses,
                    status: leadFound ? '✅ Работает' : '❌ Проблема',
                    test_url: `/api/simple-test/${student.expected_lead}`,
                    auth_url: `POST /api/auth/phone-final-fixed с body: ${JSON.stringify({ phone: student.phone, student_name: student.name })}`
                });
                
                console.log(`   ${leadFound ? '✅' : '❌'} ${leadFound ? 'Абонемент найден' : 'Проблема'}: ${totalClasses} занятий`);
                
            } catch (studentError) {
                console.log(`   ❌ Ошибка теста: ${studentError.message}`);
                results.push({
                    student: student.name,
                    status: '❌ Ошибка',
        ...
        res.json({
            ...
            results: results,
            summary: {
                total_tested: testStudents.length,
                working: results.filter(r => r.status === '✅ Работает').length,
                problems: results.filter(r => r.status === '❌ Проблема').length,
                errors: results.filter(r => r.status === '❌ Ошибка').length
            },
            recommendations: results.filter(r => r.status !== '✅ Работает').map(r => ({
                student: r.student,
                action: r.test_url ? `GET ${r.test_url} - создать профиль напрямую` : 'Проверить в amoCRM'
            }))
        });
...
app.get('/api/test-lead-correct/:leadId', async (req, res) => {
    try {
        const leadId = req.params.leadId;
        
        console.log(`\n🧪 ПРАВИЛЬНАЯ ПРОВЕРКА СДЕЛКИ ${leadId}`);
        ...
        // Создаем правильный профиль
        const studentName = amoCrmService.extractStudentNameFromLead(lead.name) || 'Ученик';
        
        const profile = {
            student_name: studentName,
            phone_number: '79660587744',
            branch: 'Чертаново',
            subscription_type: subscriptionInfo.subscriptionType,
            subscription_active: subscriptionInfo.subscriptionActive,
            subscription_status: subscriptionInfo.subscriptionStatus,
        ...
        res.json({
            success: true,
            message: 'Данные извлечены правильно',
            data: {
                lead: {
                    id: lead.id,
                    name: lead.name
...
// ==================== ПРОСТОЙ РАБОЧИЙ МАРШРУТ ДЛЯ БЫСТРОЙ ПРОВЕРКИ ====================
app.get('/api/simple-lead/:leadId', async (req, res) => {
    try {
        const leadId = req.params.leadId;
        
        console.log(`\n🔍 ПРОСТОЙ ПОИСК СДЕЛКИ: ${leadId}`);
        
        // Просто получаем сделку
        const lead = await amoCrmService.makeRequest('GET', 
            `/api/v4/leads/${leadId}`
        );
        ...
        // Базовая информация об абонементе
        const customFields = lead.custom_fields_values || [];
        let totalClasses = 0;
        let remainingClasses = 0;
        
        // Ищем поля с занятиями
        for (const field of customFields) {
            const fieldName = field.field_name || '';
            const fieldValue = amoCrmService.getFieldValue(field);
            
            if (fieldName.toLowerCase().includes('занят') && fieldName.toLowerCase().includes('всего')) {
                totalClasses = parseInt(fieldValue) || 0;
            }
            
            if (fieldName.toLowerCase().includes('остат') && fieldName.toLowerCase().includes('занят')) {
                remainingClasses = parseInt(fieldValue) || 0;
            }
        }
        
        // Создаем простой профиль
        const simpleProfile = {
            student_name: amoCrmService.extractStudentNameFromLead(lead.name) || 'Ученик',
            phone_number: '79660587744',
            branch: 'Автоматический',
            subscription_type: totalClasses > 0 ? 'Абонемент' : 'Без абонемента',
            subscription_active: totalClasses > 0 && remainingClasses > 0,
            total_classes: totalClasses,
            remaining_classes: remainingClasses,
            used_classes: totalClasses - remainingClasses,
            last_sync: new Date().toISOString()
        };
